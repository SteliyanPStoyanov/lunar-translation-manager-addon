...
namespace Lunar\TranslationManager\Drivers;

use Illuminate\Filesystem\Filesystem;
use Illuminate\Support\Arr;
use Illuminate\Support\Collection;
use Illuminate\Support\Str;
use Lunar\TranslationManager\Exception\LanguageExistsException;
...
class File extends Translation implements DriverInterface
{
   ...
   $flattenedArray = $this->arrayDotNotation($translations);

        $this->disk->put("$directory" . DIRECTORY_SEPARATOR . "{$group}.php", "<?php\n\nreturn " . varexport($flattenedArray, true) . ';' . \PHP_EOL);
    }

   public function arrayDotNotation($array, $prepend = '') {
    $results = [];

    foreach ($array as $key => $value) {
        if (is_array($value) && !empty($value)) {
            $results = array_merge($results,  $this->arrayDotNotation($value, $prepend . $key . '.'));
        } else {
